...
public class WarpPadScript : MonoBehaviour
{
    ...
    public GameObject needRedGemsPanel, dr4, levelCompletePanel, WarpingPad;
    ...
    void OnCollisionEnter2D(Collision2D col)
    {
        ...
        {
            touched = true;
            if(auroraControls.countRedGems < 78)
            {
                Debug.Log("Get some more red gems!");
                needRedGemsPanel.SetActive(true);
                touched = false;
            }  
            else if(auroraControls.countRedGems > 77)
            {
